...
class APICustomerController extends Controller
{
    ...
    public function register(APIRegisterRequest $request)
    {
        ...
        $customers->status = 0;
        $customers->remember_token = $token;
        $customers->save();

        Mail::send('emails.accept-account', compact('customers'), function ($message) use ($customers) {
        ...
        return response()->json([
            ...
            'customer' => $customers, // nếu cần trả về thông tin khách hàng sau khi đăng ký
        ]);
        
    }

    public function kiemTraTokenDangKy($customerID, $token)
    {
        $tokenInfo = DB::table('customers')
            ->where('id', $customerID)
        ...
        $expirationMinutes = env('TOKEN_EXPIRATION_MINUTES', 10);
        $expirationTime = $creationTime->addMinutes($expirationMinutes);
    
        return now()->lessThanOrEqualTo($expirationTime);
    }

    public function Accept($customerID,$token){

        if (!$this->kiemTraTokenDangKy($customerID, $token)) {
