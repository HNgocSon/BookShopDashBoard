...
class APIOrderController extends Controller
{
    ...
    public function store(APICheckoutRequest $rq)
    {
        ...
        for ($i = 0; $i < count($rq->book_id); $i++) {
            if ($rq->book_id[$i] != null && $rq->book_quantity[$i] != null) {
                $book = Book::find($rq->book_id[$i]);
                if ($book->quantity < $rq->book_quantity[$i]) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Số lượng sách ' . $book->title . ' không đủ trong kho.'
                    ]);
                }
            }
        }
        ...
        for ($i = 0; $i < count($rq->combo_id); $i++) {
            if ($rq->combo_id[$i] != null && $rq->combo_quantity[$i] != null) {
                $combo = Combo::find($rq->combo_id[$i]);
                if ($combo->quantity < $rq->combo_quantity[$i]) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Số lượng combo ' . $combo->title . ' không đủ trong kho.'
                    ]);
                }
            }
        }
